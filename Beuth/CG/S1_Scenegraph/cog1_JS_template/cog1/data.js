...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function cleanData(){		
		...
		let first;
		let second;
		let dirtyvertices= this.polygonVertices;

		for(let i=0; i<this.vertices.length;i++){
			first=this.vertices[i];

			for(let j=i+1; j<this.vertices.length;j++){
				second=this.vertices[j];
				if(checkarray(first,second)){	
					
					doublevertices.push(j);
					originalvertices.push(i);

				}
			}
		}
		//console.log(doublevertices);
		//console.log(originalvertices);

		for(let k=0; k< dirtyvertices.length;k++){
			for (let m=0; m< dirtyvertices[k].length;m++){
				for(let i=0;i<doublevertices.length;i++){
					if(dirtyvertices[k][m]===doublevertices[i]){
						dirtyvertices[k][m]=originalvertices[i];
					}
				}
			}
		}
		...
		for(let i=0; i< dirtyvertices.length;i++){
			first=dirtyvertices[i];
			for(let j=i+1; j< dirtyvertices.length;j++){
				second=dirtyvertices[j];
				if(checkarray(first,second)){
					dirtyvertices.splice(j,1);
				}
			}
		}

		//console.log(dirtyvertices);

		this.polygonVertices=dirtyvertices;

		

		//check Vertices fpr equal vertices
		//replace index
		// check for equal Polygons 
		//delete equal polygons 
		//return new polygonvertices Array
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		for(var p = 0; p < this.polygonVertices.length; p++) {
			...
			// Loop over vertices/edges in polygon.
			for(var v = 0; v < this.polygonVertices[p].length; v++) {
					let vertex =this.polygonVertices[p][v]
					//console.log(vertex);
					// Accumulate/add all vertex normals from polygonNormals.
					this.vertexNormals[vertex]= vec3.add(this.vertexNormals[vertex], this.polygonNormals[p]);	
			}
		}
	...
	/**DONE*/function calculateNormalForPolygon(vertices, polygon, n){ 
		...
		for (let i=0;i<3; i++){
			// Check for polygon vertex exist (common index error in data)------> nochmachen!
		...
		/**    Das hier produziert einen Fehler in unseren Normalen!!!!
		...
		*/
		...
		var nBetrag= Math.sqrt(Math.pow(n0,2)+Math.pow(n1,2)+Math.pow(n2,2));
